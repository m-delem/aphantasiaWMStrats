...
trial_level_cols <- c(
  ...
  "response_color",
  "live_diff_word", "live_diff_angle", "live_diff_color",
  "rt_word", "rt_angle", "rt_color",
  "feedback_score_word", "feedback_score_angle", "feedback_score_color",
  "feedback_trial_score",
...
dataset_info <- list(
  name = "WM-FTT working memory strategies data (v1/v2/v3), participant- and stimulus-level exports",
  description = paste(
    "Data from the Working Memory Feature Trade-off Task (WM-FTT):",
    "word, orientation and colour recall under a parity-judgement",
    "distractor, pooled across three task versions (v1 original, v2",
    "added a parity-error penalty, v3 added recall-order",
    "randomisation). Two datasets are described here: all_data_surveys",
    "(one row per participant: demographics, questionnaire scores, and",
    "raw questionnaire/strategy items) and all_data_full (one row per",
    "stimulus: trial structure, targets, responses, timing, and",
    "per-feature recall scores; no questionnaire item detail, see",
    "all_data_surveys for that).",
    "NAMING NOTE: the task was called CFA-WM during data collection, and",
    "column names, OSF component names and the front-end code still use",
    "that prefix (for example strats_cfa_q01_colors_1). Those",
    "identifiers are deliberately unchanged so that this export remains",
    "byte-comparable with earlier ones and with the raw JATOS data.",
    "WM-FTT and CFA-WM refer to the same task."
  ),
  sources = c(
...
surveys_core_variables <- list(
  ...
  list(
    name = "version",
    description = "WM-FTT task version.",
    dataType = "string",
    levels = c(
  ...
  list(
    name = "feedback_total_score_word",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Cumulative word points shown to the",
      "participant at the end of the task, summed across trials. Derived from",
      "the front end's defective edit distance; see live_diff_word. For",
      "analysis use score_word, aggregated as the question requires."
    ),
    dataType = "float",
    minValue = 6,
    maxValue = 70.5
  ...
  list(
    name = "feedback_total_score_angle",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Cumulative orientation points shown at",
      "the end of the task. For analysis use score_angle."
    ),
    dataType = "float",
    minValue = 0,
    maxValue = 64.5
  ...
  list(
    name = "feedback_total_score_color",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Cumulative colour points shown at the",
      "end of the task. For analysis use score_color."
    ),
    dataType = "float",
    minValue = 5,
    maxValue = 64
  ...
  list(
    name = "feedback_total_score",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Combined total points shown to the",
      "participant, across all three features and all trials, including the",
      "parity penalty in v2 and v3. This is the number participants were told",
      "to maximise, so it is meaningful as an account of what they were",
      "optimising, but it is not a performance measure."
    ),
    dataType = "float",
    minValue = 11.5,
    maxValue = 195.5
...
surveys_osivq_items <- lapply(osivq_item_names, function(nm) {
  list(
    ...
    maxValue = 5,
    notes = paste(
      "1 = totally disagree; 5 = totally agree. Missing for 2 participants",
      "overall. IMPORTANT: reverse-keyed items are stored inconsistently",
      "across versions. v1 and v2 store osivq_q02v, osivq_q09v, osivq_q41v",
      "and osivq_q42s UNREVERSED; v3 stores them already reversed. The",
      "object_mean / spatial_mean / verbal_mean subscale scores are correct",
      "in both cases; only these raw item columns differ. Recomputing",
      "subscale scores or reliabilities from the items, without reversing",
      "them for v1/v2, gives wrong answers: Cronbach's alpha for the verbal",
...
full_variables <- list(
  ...
  list(
    name = "score_word", dataType = "float", minValue = 0, maxValue = 1,
    description = paste(
      "Word recall similarity, 1 = exact match. Edit-distance scoring",
      "(Gonthier, 2022): Damerau-Levenshtein distance capped at the target's",
      "length, subtracted from it, divided by it. Computed in R from the raw",
      "target and response, not from the task's own feedback columns.",
      "WARNING: word recall is near ceiling in this task and its split-half",
      "reliability is 0.45, below the conventional floor; it does not support",
      "individual-differences claims. See the Scoring page of the package",
      "documentation."
    )
  ),
  list(
    name = "score_angle", dataType = "float", minValue = 0, maxValue = 1,
    description = paste(
      "Orientation recall similarity, 1 = exact match. Cosine of the angular",
      "error on a 180-degree period, since a rectangle's tilt is symmetric",
      "under 180-degree rotation. Split-half reliability 0.82."
    )
  ),
  list(
    name = "score_color", dataType = "float", minValue = 0, maxValue = 1,
    description = paste(
      "Colour recall similarity, 1 = exact match. Cosine of the angular error",
      "on the 360-degree hue wheel. Split-half reliability 0.83."
    )
  ),
  list(
    name = "score_word_z", dataType = "float",
    description = paste(
      "score_word, z-scored within task version using experimental-block",
      "rows only. Retained alongside the raw score: raw for absolute",
      "performance, standardised for cross-feature comparison."
    )
  ),
  list(
    name = "score_angle_z", dataType = "float",
    description = "score_angle, z-scored within task version. See score_word_z."
  ),
  list(
    name = "score_color_z", dataType = "float",
    description = "score_color, z-scored within task version. See score_word_z."
  ),

  list(
    name = "responded_word", dataType = "boolean",
    description = paste(
      "Whether the participant entered a word at all. IMPORTANT: the task",
      "encodes non-response with sentinels rather than missing values, and",
      "non-responses are scored 0 in score_word. Analyses of recall accuracy",
      "should filter on this flag; analyses that leave it out are combining",
      "accuracy with willingness to respond, which are different quantities.",
      "See the Scoring page of the package documentation."
    )
  ),
  list(
    name = "responded_angle", dataType = "boolean",
    description = paste(
      "Whether the participant moved the orientation widget. NOTE: unlike the",
      "other two features this is an inference, not an observation. The widget",
      "starts at 90 degrees and returns that value if untouched, so a",
      "deliberate 90-degree response is indistinguishable from no response.",
      "No block target lies within 20 degrees of 90, so such a response can",
      "never be near-correct. See responded_word."
    )
  ),
  list(
    name = "responded_color", dataType = "boolean",
    description = paste(
      "Whether the participant moved the colour wheel. Non-response is marked",
      "in the raw data by response_color_angle == 999 and response_color",
      "'#AAAAAA'. See responded_word."
    )
  ),

  list(
    name = "live_diff_word", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Word recall dissimilarity as computed",
      "by the task's front end to drive its feedback screen, 0 = exact match.",
      "Note this runs opposite to score_word. WARNING: the front end's edit",
      "distance is defective (a loop whose condition is an assignment leaves",
      "the distance matrix half-initialised), so this is not a valid edit",
      "distance. Retained for provenance only. See the Scoring page."
    )
  ),
  list(
    name = "live_diff_angle", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Absolute orientation error divided by",
      "180, as computed by the front end, 0 = exact match. Linear rather than",
      "cosine, and with no handling of the rectangle's 180-degree symmetry.",
      "Use score_angle instead."
    )
  ),
  list(
    name = "live_diff_color", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Circular colour error divided by 180,",
      "as computed by the front end, 0 = exact match. Non-responses are mapped",
      "to 1 by an explicit guard. Use score_color instead."
    )
  ),
  ...
  list(
    name = "feedback_score_word", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Points shown on the feedback screen",
      "for the word response (0, 0.5 or 1), thresholded from live_diff_word",
      "and therefore inheriting its defective edit distance. Use score_word."
    )
  ),
  list(
    name = "feedback_score_angle", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Points shown for the orientation",
      "response (0, 0.5 or 1). Use score_angle."
    )
  ),
  list(
    name = "feedback_score_color", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Points shown for the colour response",
      "(0, 0.5 or 1). Use score_color."
    )
  ),
  list(
    name = "feedback_trial_score", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Points shown for the whole trial: the",
      "sum of the three per-feature feedback scores, minus 0.5 per incorrect",
      "parity judgement, floored at 0. Trial-level, so constant across the",
      "three stimulus rows of a trial. The parity penalty applies in v2 and v3",
      "only."
    )
  ),
  ...
  list(
    name = "feedback_total_score_word", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Cumulative word points shown to the",
      "participant at the end of the task. Derived from the front end's",
      "defective edit distance; see live_diff_word. Use score_word."
    )
  ),
  list(
    name = "feedback_total_score_angle", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Cumulative orientation points shown at",
      "the end of the task. Use score_angle."
    )
  ),
  list(
    name = "feedback_total_score_color", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Cumulative colour points shown at the",
      "end of the task. Use score_color."
    )
  ),
  list(
    name = "feedback_total_score", dataType = "float",
    description = paste(
      "DISPLAY ONLY, NOT FOR ANALYSIS. Combined total points shown to the",
      "participant, including the parity penalty in v2 and v3. This is the",
      "number participants were told to maximise, so it is meaningful as an",
      "account of what they were optimising, but it is not a performance",
      "measure."
    )
  ),
