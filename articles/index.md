# Articles

### What was measured

The paradigm, and how a response becomes a number. Everything after this
depends on it.

- [The participants: who took
  part](https://m-delem.github.io/aphantasiaWMStrats/articles/participants.md):
- [Task design: the story of a paradigm that kept correcting
  itself](https://m-delem.github.io/aphantasiaWMStrats/articles/task-design.md):
- [Scoring the WM-FTT: what the numbers are, and what they
  support](https://m-delem.github.io/aphantasiaWMStrats/articles/scoring.md):
- [Task engagement: two ways of not doing the
  task](https://m-delem.github.io/aphantasiaWMStrats/articles/engagement.md):

### Scope

Which task versions enter the analyses, and why the rest do not. Every
result downstream inherits this restriction.

- [Versions: which one the analyses use, and
  why](https://m-delem.github.io/aphantasiaWMStrats/articles/version-scope.md):

### Does anything here measure anything?

Reliability and validity, for the task and for the questionnaires. Both
have to hold before an association between them means anything.

- [Task validity: what the WM-FTT measures well, and what it does
  not](https://m-delem.github.io/aphantasiaWMStrats/articles/task-validity.md):
- [Psychometrics: how the questionnaires behave in this
  sample](https://m-delem.github.io/aphantasiaWMStrats/articles/psychometrics.md):

### Confirmatory modelling

Imagery groups fixed in advance. The strategy page says what is modelled
and why before any results appear.

- [The analysis: what is modelled, and
  why](https://m-delem.github.io/aphantasiaWMStrats/articles/analysis-strategy.md):
- [Composition: how effort is divided, independently of how much there
  is](https://m-delem.github.io/aphantasiaWMStrats/articles/composition.md):
- [Performance: how well each feature is
  recalled](https://m-delem.github.io/aphantasiaWMStrats/articles/performance.md):
- [The joint model: reporting and accuracy
  together](https://m-delem.github.io/aphantasiaWMStrats/articles/joint-model.md):

### Exploratory

Groups not fixed in advance. Whether the questionnaires contain
structure the vividness split misses.

- [Beyond vividness: what the questionnaires
  say](https://m-delem.github.io/aphantasiaWMStrats/articles/beyond-vividness.md):

### Technical

How the models were built and checked, and what was tried and withdrawn
along the way.

- [Model
  diagnostics](https://m-delem.github.io/aphantasiaWMStrats/articles/model-diagnostics.md):
- [Implementation
  notes](https://m-delem.github.io/aphantasiaWMStrats/articles/implementation-notes.md):
- [What was tried and
  withdrawn](https://m-delem.github.io/aphantasiaWMStrats/articles/lessons.md):

### Reference

- [Codebook](https://m-delem.github.io/aphantasiaWMStrats/articles/codebook.md):
