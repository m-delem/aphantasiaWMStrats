...
responded items only, which carries a caveat set out in section 8.
...
whom answered very few orientation items, and choosing a partition on
...
independent of the result. Two rules, both fixed before any allocation
model was fitted: a scale correlating above 0.6 with VVIQ is excluded as
re-entering vividness under another name, and a scale with no hypothesis
...
is `r fmt(style_rope$Estimate[1])` with
`r fmt(100 * style_rope[["Inside ROPE"]][1], 0)`% of its posterior inside
the ROPE; OSIVQ spatial on the colour-orientation coordinate is
`r fmt(style_rope$Estimate[2])` with
`r fmt(100 * style_rope[["Inside ROPE"]][2], 0)`% inside. A verbaliser does
...
is informative rather than empty. It correlates
...
settling it: the elpd difference is smaller than its own standard error.
At that separation the data do not distinguish them, which is what
`r nrow(style_data)` participants and four extra predictors should be
expected to produce.
...
The same questionnaire asked something harder. For each feature, *how did
you memorise it*, with mental imagery among the options. Naming imagery
there is a claim about **experience**, not about behaviour, and it is the
kind of report aphantasia research treats as least trustworthy.
...
feature.** Above the floor,
`r percent_of("Word", "mental_image", "Above floor")`% named it for words,
`r percent_of("Colour", "mental_image", "Above floor")`% for colours and
`r percent_of("Orientation", "mental_image", "Above floor")`% for
orientations.
...
nothing here reaches the stronger claim. No model is fitted either: one
cell is a structural zero, so a logistic model of the group contrast would
not converge, and the table already says what the data support.

Two further things in that table, neither about imagery. On orientation,
**`r percent_of("Orientation", "none", "VVIQ floor")`% of the floor group
named no strategy at all**, against
`r percent_of("Orientation", "none", "Above floor")`% above it —
orientation is the feature they abandon, which converges with its being the
feature with the highest non-response rate
([task engagement](engagement.html)). And they named body-centred spatial
strategies **less** often than typical imagers,
`r percent_of("Orientation", "spatial_body", "VVIQ floor")`% against
`r percent_of("Orientation", "spatial_body", "Above floor")`%, which cuts
against a simple compensation account in which the missing visual strategy
is replaced by a spatial one.

**The agreement holds on the parts, not only on the summary coordinate.**
The one contrast with enough participants to look at is words alone against
words-and-colours:
...
**And prioritisation is unrelated to imagery group**, which is what makes
...
here is built from items a participant chose to answer, which conditions
