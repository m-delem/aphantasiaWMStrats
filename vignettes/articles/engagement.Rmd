...
Two of those objects are used before the page that defines them, so they are
stated here rather than left implicit. `engaged_ids()` keeps the participants
who answered at least `r wm_thresholds()[["word"]]` word,
`r wm_thresholds()[["angle"]]` orientation and
`r wm_thresholds()[["color"]]` colour items of the 63 presented. The
[task validity](task-validity.html) page derives those numbers and explains
what they are for; here they serve only to separate *abandoning the
distractor* from *abandoning the task*.
...
own right, rather than as a control for something it does not predict. The
[composition](composition.html) page carries it, and the [joint
model](joint-model.html) carries it on its accuracy arms. The
[performance](performance.html) page does **not**: every model there is
fitted without it, so that page's internal comparison holds the
right-hand side constant, but no argument was recorded for dropping it
rather than adding it to both. That is an open inconsistency rather than a
decision, and it is flagged here rather than smoothed over.
...
model](joint-model.html) fits whether a feature was reported and how well
it was reported together, rather than conditioning on the first and
analysing only the second.
