...
conditions on responded trials, and whether that is innocent is tested on
...
```{r families}
performance_formula(parity = TRUE)
```

`parity = FALSE` is worth naming, because it is an exception. The
[composition](composition.html) and [joint model](joint-model.html) pages
both carry `parity_rate`, and [task engagement](engagement.html) §5 argues
it belongs in the accuracy models as a strategy variable. Every model on
*this* page omits it, which keeps the right-hand side constant across the
comparison section 2 runs, but no reason was recorded for resolving the
divergence this way round rather than the other. It is a known loose end.
...
c_prime_priors <- c(
  do.call(c, lapply(
    ...
    response_priors, terms = c("vviq", "complete_aphant")
  )),
...
form_models <- list(
  "Intercept only" = fit_brms_model(
    formula = brms::bf(score_angle | subset(responded_angle) ~ 1 + (1 | id),
                       family = brms::Beta()),
    data = model_data,
    prior = univariate_priors("1 + (1 | id)"),
  ...
  "Two-group split" = fit_brms_model(
    formula = brms::bf(
      ...
      family = brms::Beta()),
    data = model_data,
    prior = univariate_priors("imagery_group + (1 | id)"),
  ...
  "Linear vividness" = fit_brms_model(
    formula = brms::bf(score_angle | subset(responded_angle) ~ vviq + (1 | id),
                       family = brms::Beta()),
    data = model_data,
    prior = univariate_priors("vviq + (1 | id)"),
