...
validity](task-validity.html) page, where the split-half machinery lives.
Everything below is computed when this page is built.

**This page stays on v1**, where the [beyond
vividness](beyond-vividness.html) page pools all three versions. The two
are asking different questions. Pooling is right when the aim is to
describe the instruments in as many people as possible; here the aim is to
say whether the scales are trustworthy *in the sample the modelling pages
use*, and that sample is v1. Where a claim below rests on the other
versions, it says so.
...
a plain continuous predictor everywhere downstream; [the analysis
strategy](analysis-strategy.html) page §3 sets out the floor-group form
every model uses and the caution that comes with it.
...
reversed, its item-total correlations for those items should be **positive**
where v1's are negative.
