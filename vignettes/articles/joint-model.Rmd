...
result, and it computes it from **responded items only**. Whether that
...
abstention: one answered **zero** orientation items, which makes them
...
The `subset()` terms are what let one item carry six responses with
...
It replicates: estimated from orientation alone it is
`r fmt(selection_replication[["orientation_alone"]], 2)`, and with all six
responses in the model
`r fmt(selection_replication[["six_responses"]], 2)`, so it is not an
artifact of the smaller specification. Both are read from the fitted
...
removes guaranteed-wrong items from the accuracy of exactly the
...
Non-response is rare, between 7% and 14% of items, and the floor group is
...
That is the expected direction: with the gates in the model, variance
previously absorbed into accuracy is attributed to willingness instead.
...
diagnostics](model-diagnostics.html#convergence) page, alongside the rest
of the convergence check and the posterior predictive checks. They are
repeated here because a reader should not have to leave the page reporting
a result to find out whether the model that produced it sampled properly.
